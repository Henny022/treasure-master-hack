...
generate_schedule_entry(uint8_t map, key_schedule_data *current_schedule_data)
{
    ...
    {
        algorithm_number =
            (current_schedule_data->as_uint8[(map >> 4) & 0x03] >> 2) & 0x07;
    }
                        ...
                        uint8_t algorithm)
{
    ...
    result.nibble_selector = (current_schedule_data->as_uint8[3] << 8) +
                             current_schedule_data->as_uint8[2];
                              ...
                              key_schedule_data *current_schedule_data)
{
    ...
    unsigned char carry =
        (((int)temp + current_schedule_data->as_uint8[0]) >> 8) & 0x01;
    temp = temp + current_schedule_data->as_uint8[0];
    unsigned char next_carry =
        (((int)temp - current_schedule_data->as_uint8[2] - (1 - carry)) < 0
             ? 0
             : 1);
    ...
    {
        unsigned char next_carry =
            (((int)rolling_sum + current_schedule_data->as_uint8[i] + carry) >>
             8) &
            0x01;
