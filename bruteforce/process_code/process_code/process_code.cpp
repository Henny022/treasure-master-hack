...
{
    // Usage:
    //  -k, --key "XXXXXXXX": supply 8 hex values to use as the key value,
    //  defaults to all 0's? -d, --data "XXXXXXXX": supply 8 hex values to use
    //  as the starting data value, defaults to all 0's -c, --count -a, --attack
    //  attack_type:
    //               "individual": each IV is processed and checked individually
    //               "vector":     each working code is added to a std::vector,
    //               processed one round, sorted, then std::unique'd "hash":
    //               each working code is processed a round, then checked
    //               against a hash table. collisions are not processed any
    //               further
    //  -b, --big-registers: use 128bit registers as an optimization
    //  -s, --key-schedule: pre-process the key schedule as an optimization
    //  instead of processing it for every IV -f, --file filename: read a list
    //  of IVs to process from a file. Overrides -k and -d
    ...
    uint64_t IVs_to_run = command_line_options.iv_count;

    uint64_t IVs_finished = 0;
    ...
    if (command_line_options.attack == INDIVIDUAL)
    {
        while (true)
        {
            ...
            {
                in_progress.process_map_exit(
                    map_list[map_index], schedule_entries[schedule_counter]);

                if (map_list[map_index] == 0x22)
                {
                    in_progress.process_map_exit(
                        map_list[map_index],
                        schedule_entries[schedule_counter + 1]);
                }
    ...
    else if (command_line_options.attack == VECTOR)
    {
        ...
        {
            uint64_t count = 0;
            for (std::vector<working_code>::iterator it =
                     IVs_in_progress.begin();
                 it != IVs_in_progress.end(); ++it)
            {
                it->process_map_exit(map_list[map_index],
                                     schedule_entries[schedule_counter]);
                ...
                {
                    // TODO
                    fraction_done((((double)count) +
                                   ((double)(map_index *
                                             command_line_options.iv_count))) /
                                  (((double)26) *
                                   ((double)command_line_options.iv_count)));
                }
            }
            ...
            IVs_in_progress.erase(
                std::unique(IVs_in_progress.begin(), IVs_in_progress.end()),
                IVs_in_progress.end());
            // shrink to fit
        ...
        for (std::vector<working_code>::iterator it = IVs_in_progress.begin();
             it != IVs_in_progress.end(); ++it)
        {
            output_stats(&(*it));
        }
    ...
    else if (command_line_options.attack == VECTOR_CONSTANT_SIZE)
    {
        ...
        for (int map_index = 0; map_index < 26; map_index++)
        {
                   ...
                   !no_more)
            {
                ...
                printf("want: %lu\n", command_line_options.vector_size -
                                          IVs_in_progress.size());
                while ((IVs_in_progress.size() + new_IVs.size()) <
                       command_line_options.vector_size)
                {
                    // Get a key
                    if (command_line_options.from_file)
                    ...
                    else
                    {
                        ...
                        {
                            value[0] =
                                (command_line_options.start_key >> 24) & 0xFF;
                            value[1] =
                                (command_line_options.start_key >> 16) & 0xFF;
                            value[2] =
                                (command_line_options.start_key >> 8) & 0xFF;
                            value[3] = command_line_options.start_key & 0xFF;

                            value[4] =
                                (command_line_options.start_data >> 24) & 0xFF;
                            value[5] =
                                (command_line_options.start_data >> 16) & 0xFF;
                            value[6] =
                                (command_line_options.start_data >> 8) & 0xFF;
                            value[7] = command_line_options.start_data & 0xFF;
                        }
                        else
                        {
                    ...
                    if (IVs_finished == 0)
                    {
                        ...
                        {
                            schedule_entries[schedule_counter++] =
                                generate_schedule_entry(map_list[i],
                                                        &schedule_data);
                ...
                if (new_IVs.size() > 0)
                {
                    ...
                    for (int sub_map_index = 0; sub_map_index < map_index;
                         sub_map_index++)
                    {
                        ...
                        {
                            it->process_map_exit(
                                map_list[sub_map_index],
                                schedule_entries[sub_schedule_counter]);

                            if (map_list[sub_map_index] == 0x22)
                            {
                                it->process_map_exit(
                                    map_list[sub_map_index],
                                    schedule_entries[sub_schedule_counter + 1]);
                            }
                        }
                        ...
                        new_IVs.erase(
                            std::unique(new_IVs.begin(), new_IVs.end()),
                            new_IVs.end());
                        // shrink to fit
                ...
                IVs_in_progress.insert(IVs_in_progress.end(), new_IVs.begin(),
                                       new_IVs.end());
                new_IVs.empty();
        ...
        while (IVs_finished < command_line_options.iv_count)
        {
            ...
            for (int map_index = 0; map_index < 26; map_index++)
            {
                if (map_index == 0)
                {
                           ...
                           command_line_options.vector_size)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                value[0] =
                                    (command_line_options.start_key >> 24) &
                                    0xFF;
                                value[1] =
                                    (command_line_options.start_key >> 16) &
                                    0xFF;
                                value[2] =
                                    (command_line_options.start_key >> 8) &
                                    0xFF;
                                value[3] =
                                    command_line_options.start_key & 0xFF;

                                value[4] =
                                    (command_line_options.start_data >> 24) &
                                    0xFF;
                                value[5] =
                                    (command_line_options.start_data >> 16) &
                                    0xFF;
                                value[6] =
                                    (command_line_options.start_data >> 8) &
                                    0xFF;
                                value[7] =
                                    command_line_options.start_data & 0xFF;
                            }
                            else
                            {
                                // Increment to the next IV
                        ...
                        if (IVs_finished == 0)
                        {
                            ...
                            for (int i = 0; i < 26; i++)
                            {
                                schedule_entries[schedule_counter_gen++] =
                                    generate_schedule_entry(map_list[i],
                                                            &schedule_data);
                                ...
                                {
                                    schedule_entries[schedule_counter_gen++] =
                                        generate_schedule_entry(
                                            map_list[i], &schedule_data, 4);
                                }
                            }
                        }
                ...
                else
                {
                    ...
                    size_t x = IVs_in_progress.size();
                    std::sort(IVs_in_progress.begin(), IVs_in_progress.end());
                    IVs_in_progress.erase(std::unique(IVs_in_progress.begin(),
                                                      IVs_in_progress.end()),
                                          IVs_in_progress.end());
                    // shrink to fit
                    std::vector<working_code>(IVs_in_progress)
                    ...
                    if (map_index == 4)
                    {
                        ...
                        std::vector<working_code>::iterator it2 =
                            IVs_in_progress.begin();
