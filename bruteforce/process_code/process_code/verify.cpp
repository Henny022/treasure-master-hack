...
uint8_t carnival_world_working_code[0x80] = {
...
uint8_t carnival_code_decrypted_machine_code[0x80] = {
...
uint8_t opcode_bytes_used[0x100] = {
...
uint8_t *decrypt_memory(uint8_t *working_code, uint8_t *encrypted_memory,
                        int length)
{
    uint8_t *decrypted_memory = new uint8_t[128];
...
void output_stats(working_code *in_progress)
{
    uint8_t *decrypted_memory =
        decrypt_memory(in_progress->working_code_data.as_uint8, carnival_code,
                       carnival_code_length);
    ...
    if (verify_checksum(decrypted_memory, carnival_code_length))
    {
        ...
        boinc_log(
            ...
            in_progress->starting_value[0], in_progress->starting_value[1],
            in_progress->starting_value[2], in_progress->starting_value[3],
            in_progress->starting_value[4], in_progress->starting_value[5],
            in_progress->starting_value[6], in_progress->starting_value[7]);

        boinc_log("car\t");

        if (compare_working_code(in_progress->working_code_data.as_uint8,
                                 carnival_world_working_code))
        {
            boinc_log("wcm");
        }
        boinc_log("\t");
    ...
    decrypted_memory =
        decrypt_memory(in_progress->working_code_data.as_uint8,
                       other_world_code, other_world_code_length);
