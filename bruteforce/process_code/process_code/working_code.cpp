...
working_code::working_code(uint8_t value[8])
{
    ...
    {
        working_code_data.as_uint8[i] =
            working_code_data.as_uint8[i - 8] + rng.run();
    }
}
...
void working_code::process_map_exit(uint8_t map_number,
                                    key_schedule_entry schedule_entry)
{
    process_working_code(map_number, schedule_entry);
}

void working_code::process_working_code(uint8_t map_number,
                                        key_schedule_entry schedule_entry)
{
    ...
    // printf("rng1: %02x, rng2: %02X, nibble:
    ...
    // Somehow the game decides which map to go to next, and our work here is
    // done
}

void working_code::working_code_algorithm(uint8_t algorithm_number,
                                          uint8_t map_number)
{
    // printf("alg #: %i\n",algorithm_number);
    ...
    if (algorithm_number == 0x00)
    {
        ...
        for (int i = 0x7F; i >= 0; i--)
        {
            ...
            // ROL $0200,X
            working_code_data.as_uint8[i] =
                (working_code_data.as_uint8[i] << 1) |
                ((rng.run() >> 7) & 0x01);
    ...
    else if (algorithm_number == 0x01)
    {
        ...
        for (int i = 0x7F; i >= 0; i--)
        {
            ...
            // STA $0200,X
            working_code_data.as_uint8[i] =
                working_code_data.as_uint8[i] + rng.run();
    ...
    else if (algorithm_number == 0x02)
    {
        ...
        for (int i = 0x7F; i >= 0; i -= 2)
        {
            ...
            // DEX
            working_code_data.as_uint8[i - 1] =
                (working_code_data.as_uint8[i - 1] >> 1) |
                (working_code_data.as_uint8[i] & 0x80);
            working_code_data.as_uint8[i] =
                (working_code_data.as_uint8[i] << 1) | (carry & 0x01);
    ...
    else if (algorithm_number == 0x04)
    {
        ...
        for (int i = 0x7F; i >= 0; i--)
        {
            ...
            // STA $0200,X
            working_code_data.as_uint8[i] =
                working_code_data.as_uint8[i] + (rng.run() ^ 0xFF) + 1;
    ...
    else if (algorithm_number == 0x05)
    {
        ...
        for (int i = 0x7F; i >= 0; i -= 2)
        {
            ...
            working_code_data.as_uint8[i - 1] =
                (working_code_data.as_uint8[i - 1] << 1) |
                (working_code_data.as_uint8[i] & 0x01);
            working_code_data.as_uint8[i] =
                (working_code_data.as_uint8[i] >> 1) | carry;
    ...
    else if (algorithm_number == 0x06)
    {
        ...
        for (int i = 0x00; i <= 0x7f; i++)
        {
            ...
            // ROR $0200,X
            working_code_data.as_uint8[i] =
                (working_code_data.as_uint8[i] >> 1) | (rng.run() & 0x80);
...
bool working_code::operator==(const working_code &other) const
{
    for (int i = 0; i < 16; i++)
    {
        if (working_code_data.as_uint64[i] !=
            other.working_code_data.as_uint64[i])
        {
            return false;
        }
    }
...
bool working_code::operator<(const working_code &other) const
{
    for (int i = 0; i < 16; i++)
    {
        if (working_code_data.as_uint64[i] !=
            other.working_code_data.as_uint64[i])
        {
            return (working_code_data.as_uint64[i] <
                    other.working_code_data.as_uint64[i]);
        }
    }
    return false;
