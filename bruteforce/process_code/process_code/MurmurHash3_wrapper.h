...
struct MurmurHashKey
{
    ...
    MurmurHashKey(uint32_t val) : value(val) {}
...
{
    // printf("\nrunning murmur hash on IV:
    // %02X%02X%02X%02X%02X%02X%02X%02X\t\t",to_hash.starting_value[0],to_hash.starting_value[1],to_hash.starting_value[2],to_hash.starting_value[3],to_hash.starting_value[4],to_hash.starting_value[5],to_hash.starting_value[6],to_hash.starting_value[7]);

    uint32_t key;
    MurmurHash3_x86_32(to_hash.working_code_data.as_uint8, 128, hash_seed,
                       &key);
    // printf("%08X\n",key);
...
template <> struct hash<MurmurHashKey>
{
  public:
