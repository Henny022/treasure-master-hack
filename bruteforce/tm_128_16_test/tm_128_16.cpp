          ...
          const uint16_t rng_seed)
{
    for (int i = 0; i < 16; i++)
    {
        ...
        __m128i rng_val = _mm_loadu_si128(
            (__m128i *)(alg0_values + (rng_seed * 128 * 2) + (i * 16)));
          ...
          const uint16_t rng_seed)
{
    for (int i = 0; i < 16; i++)
    {
        __m128i cur_val = _mm_loadu_si128((__m128i *)(working_code + i * 16));
        __m128i rng_val = _mm_loadu_si128(
            (__m128i *)(regular_rng_values + (rng_seed * 128 * 2) + (i * 16)));
          ...
          const uint16_t rng_seed)
{
    ...
    for (int i = 15; i >= 0; i--)
    {
        ...
        __m128i next_carry = _mm_slli_si128(
            _mm_and_si128(cur_val, _mm_set_epi16(0, 0, 0, 0, 0, 0, 0, 1)), 14);

        carry = _mm_or_si128(
            carry,
            _mm_srli_si128(
                _mm_and_si128(cur_val, _mm_set_epi16(0, 1, 0, 1, 0, 1, 0, 0)),
                2));
        ...
        cur_val = _mm_or_si128(
            _mm_or_si128(
                _mm_or_si128(_mm_and_si128(_mm_srli_epi16(cur_val, 1),
                                           _mm_set_epi16(0, 0x7f, 0, 0x7f, 0,
                                                         0x7f, 0, 0x7f)),
                             _mm_and_si128(_mm_slli_epi16(cur_val, 1),
                                           _mm_set_epi16(0xFE, 0, 0xFE, 0, 0xFE,
                                                         0, 0xFE, 0))),
                _mm_and_si128(
          ...
          const uint16_t rng_seed)
{
    ...
    for (int i = 15; i >= 0; i--)
    {
        ...
        carry = _mm_or_si128(
            carry, _mm_srli_si128(
                       _mm_and_si128(cur_val, _mm_set_epi16(0, 0x80, 0, 0x80, 0,
                                                            0x80, 0, 0)),
                       2));
        ...
        cur_val = _mm_or_si128(
            _mm_or_si128(
                _mm_or_si128(_mm_and_si128(_mm_srli_epi16(cur_val, 1),
                                           _mm_set_epi16(0x7f, 0, 0x7f, 0, 0x7f,
                                                         0, 0x7f, 0)),
                             _mm_and_si128(_mm_slli_epi16(cur_val, 1),
                                           _mm_set_epi16(0, 0xFE, 0, 0xFE, 0,
                                                         0xFE, 0, 0xFE))),
                _mm_and_si128(_mm_srli_si128(cur_val, 2),
          ...
          const uint16_t rng_seed)
{
    for (int i = 0; i < 16; i++)
    {
        __m128i cur_val = _mm_loadu_si128((__m128i *)(working_code + i * 16));
        __m128i rng_val = _mm_loadu_si128(
            (__m128i *)(alg6_values + (rng_seed * 128 * 2) + (i * 16)));
