             ...
             uint16_t *rng_forward_128)
{
    ...
    else if (alg_id == 2)
    {
        for (int j = 0; j < iterations; j++)
        {
            ...
            __m256i next_carry = _mm256_and_si256(
                _mm256_set_m128i(_mm_slli_si128(cur_val_lo, 14), cur_val_lo),
                mask_top_01);
            __m128i temp_lo = _mm_or_si128(_mm_srli_si128(cur_val_lo, 2),
                                           _mm_slli_si128(cur_val_hi, 14));
            ...
            working_code7 = _mm256_or_si256(
                carry, _mm256_and_si256(cur_val_srl, mask_0080));
            carry = next_carry;

            cur_val_lo = _mm256_castsi256_si128(working_code6);
            cur_val_hi = _mm256_extractf128_si256(working_code6, 1);
            next_carry = _mm256_and_si256(
                _mm256_set_m128i(_mm_slli_si128(cur_val_lo, 14), cur_val_lo),
                mask_top_01);
            temp_lo = _mm_or_si128(_mm_srli_si128(cur_val_lo, 2),
                                   _mm_slli_si128(cur_val_hi, 14));
    ...
    else if (alg_id == 5)
    {
        for (int j = 0; j < iterations; j++)
        {
            ...
            working_code7 = _mm256_or_si256(
                carry, _mm256_and_si256(cur_val_srl, mask_0001));
            carry = next_carry;
