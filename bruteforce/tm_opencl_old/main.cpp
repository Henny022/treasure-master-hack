...
int main()
{
    ...
    ret = clGetDeviceIDs(platform_id, CL_DEVICE_TYPE_DEFAULT, 2, &device_id,
                         &ret_num_devices);
    if (ret != CL_SUCCESS)
    {
    ...
    rng_space_d = clCreateBuffer(context, CL_MEM_READ_WRITE,
                                 sizeof(struct rng_data) * 0x10000, NULL, &ret);
    if (ret != CL_SUCCESS)
    {
    ...
    generate_rng_table();
    struct rng_data *rng_space_h =
        (struct rng_data *)malloc(sizeof(struct rng_data) * 0x10000);
    ...
    for (int i = 0; i < 0x10000; i++)
    {
        ...
        for (int j = 31; j >= 0; j--)
        {
            ...
            rng_space_h[i].regular_values[j] =
                rng_space_h[i].regular_values[j] | (rng.run() << 16);
            rng_space_h[i].regular_values[j] =
                rng_space_h[i].regular_values[j] | (rng.run() << 8);
            rng_space_h[i].regular_values[j] =
                rng_space_h[i].regular_values[j] | rng.run();
        }

        rng.seed((rng_seed >> 8) & 0xFF, rng_seed & 0xFF);
        uint8_t expansion_values[8] = {0, 0, 0, 0, 0, 0, 0, 0};
        ...
        for (int j = 1; j < 16; j++)
        {
            ...
            rng_space_h[i].expansion_values_high[j * 2] =
                (expansion_values[3] << 24) | (expansion_values[1] << 8);
            rng_space_h[i].expansion_values_low[j * 2] =
                (expansion_values[2] << 16) | expansion_values[0];

            rng_space_h[i].expansion_values_high[j * 2 + 1] =
                (expansion_values[7] << 24) | (expansion_values[5] << 8);
            rng_space_h[i].expansion_values_low[j * 2 + 1] =
                (expansion_values[6] << 16) | expansion_values[4];
        }

        for (int j = 0; j < 32; j++)
        {
            rng_space_h[i].alg0_values[j] =
                (rng_space_h[i].regular_values[j] & 0x80808080) >> 7;

            // Alg6 is not reversed..wtf
            rng_space_h[i].alg6_values[j] =
                rng_space_h[i].regular_values[31 - j] & 0x80808080;
            rng_space_h[i].alg6_values[j] =
                ((rng_space_h[i].alg6_values[j] & 0xFF) << 24) |
                ((rng_space_h[i].alg6_values[j] & 0xFF00) << 8) |
                ((rng_space_h[i].alg6_values[j] & 0xFF0000) >> 8) |
                ((rng_space_h[i].alg6_values[j] & 0xFF000000) >> 24);

            rng_space_h[i].arth_high_values[j] =
                rng_space_h[i].regular_values[j] & 0xFF00FF00;
            rng_space_h[i].arth_low_values[j] =
                rng_space_h[i].regular_values[j] & 0x00FF00FF;
        }
        ...
        rng_space_h[i].expansion_values_low[j*2],
        rng_space_h[i].expansion_values_high[j*2+1],
        rng_space_h[i].expansion_values_low[j*2+1]); printf("%02X ",
        rng_space_h[i].alg0_values[j]);
                }
    ...
    ret = clEnqueueWriteBuffer(command_queue, rng_space_d, CL_TRUE, 0,
                               sizeof(struct rng_data) * 0x10000, rng_space_h,
                               0, NULL, NULL);
    if (ret != CL_SUCCESS)
    {
    ...
    code_space_d = clCreateBuffer(context, CL_MEM_READ_WRITE,
                                  sizeof(unsigned char) * 8, NULL, &ret);
    if (ret != CL_SUCCESS)
    {
        printf("Error in clCreateBuffer, Line %u in file %s !!!\n\n", __LINE__,
               __FILE__);
    ...
    ret = clEnqueueWriteBuffer(command_queue, code_space_d, CL_TRUE, 0, 8,
                               code_space_h, 0, NULL, NULL);
    if (ret != CL_SUCCESS)
    {
        printf("Error in clEnqueueWriteBuffer, Line %u in file %s !!!\n\n",
               __LINE__, __FILE__);
    ...
    key_schedule_d =
        clCreateBuffer(context, CL_MEM_READ_WRITE,
                       sizeof(key_schedule_entry_new) * 27, NULL, &ret);
    if (ret != CL_SUCCESS)
    {
        printf("Error in clCreateBuffer, Line %u in file %s !!!\n\n", __LINE__,
               __FILE__);
    ...
    for (int i = 0; i < 27; i++)
    {
        schedule_entries_new[i].rng_seed =
            (schedule_entries[i].rng1 << 8) | schedule_entries[i].rng2;
        schedule_entries_new[i].nibble_selector =
            schedule_entries[i].nibble_selector;
    }

    ret = clEnqueueWriteBuffer(command_queue, key_schedule_d, CL_TRUE, 0,
                               sizeof(key_schedule_entry_new) * 27,
                               schedule_entries_new, 0, NULL, NULL);
    if (ret != CL_SUCCESS)
    {
        printf("Error in clEnqueueWriteBuffer, Line %u in file %s !!!\n\n",
               __LINE__, __FILE__);
    ...
    result_space_d = clCreateBuffer(context, CL_MEM_READ_WRITE,
                                    sizeof(unsigned char) * 128, NULL, &ret);
    if (ret != CL_SUCCESS)
    {
        printf("Error in clCreateBuffer, Line %u in file %s !!!\n\n", __LINE__,
               __FILE__);
    ...
    unsigned char *result_space_h =
        (unsigned char *)malloc(sizeof(unsigned char) * 128);
    for (int i = 0; i < 128; i++)
    {
        result_space_h[i] = 0;
    }

    ret = clEnqueueWriteBuffer(command_queue, result_space_d, CL_TRUE, 0, 128,
                               result_space_h, 0, NULL, NULL);
    if (ret != CL_SUCCESS)
    {
        printf("Error in clEnqueueWriteBuffer, Line %u in file %s !!!\n\n",
               __LINE__, __FILE__);
    ...
    unsigned int blah = 0;
    ret = clEnqueueReadBuffer(command_queue, code_space_d, CL_TRUE, 0, 4, &blah,
                              0, NULL, NULL);
    if (ret != CL_SUCCESS)
    {
    ...
    ret = clEnqueueReadBuffer(command_queue, result_space_d, CL_TRUE, 0, 128,
                              result_space_h, 0, NULL, NULL);
    if (ret != CL_SUCCESS)
    {
        printf("(%i) Error in clEnqueueReadBuffer, Line %u in file %s !!!\n\n",
    ...
    size_t bin_sz;
    ret = clGetProgramInfo(program, CL_PROGRAM_BINARY_SIZES, sizeof(size_t),
                           &bin_sz, NULL);

    // Read binary (PTX file) to memory buffer
    unsigned char *bin = (unsigned char *)malloc(bin_sz);
    ret = clGetProgramInfo(program, CL_PROGRAM_BINARIES,
