...
void main()
{
    ...
    uint16_t rng_seed;
    uint16_t *rng_table = new uint16_t[256 * 256];
    generate_rng_table(rng_table);

    uint8_t *regular_rng_values =
        (uint8_t *)aligned_malloc(0x10000 * 128 * 2, 32);
    generate_regular_rng_values_16((uint16_t *)regular_rng_values, rng_table);

    uint8_t *alg0_values = (uint8_t *)aligned_malloc(0x10000 * 128 * 2, 32);
    generate_alg0_values_16((uint16_t *)alg0_values, rng_table);

    uint8_t *alg6_values = (uint8_t *)aligned_malloc(0x10000 * 128 * 2, 32);
    generate_alg6_values_16((uint16_t *)alg6_values, rng_table);

    uint8_t *alg2_values = (uint8_t *)aligned_malloc(0x10000 * 32, 32);
    generate_alg2_values_256_16(alg2_values, rng_table);

    uint8_t *alg5_values = (uint8_t *)aligned_malloc(0x10000 * 32, 32);
    generate_alg5_values_256_16(alg5_values, rng_table);

    uint16_t *rng_forward_1 = new uint16_t[256 * 256];
    generate_seed_forward_1(rng_forward_1, rng_table);

    uint16_t *rng_forward_128 = new uint16_t[256 * 256];
    generate_seed_forward_128(rng_forward_128, rng_table);

    int map_list[26] = {0x00, 0x02, 0x05, 0x04, 0x03, 0x1D, 0x1C, 0x1E, 0x1B,
                        0x07, 0x08, 0x06, 0x09, 0x0C, 0x20, 0x21, 0x22, 0x23,
                        0x24, 0x25, 0x26, 0x0E, 0x0F, 0x10, 0x12, 0x11};

    uint8_t IV[4] = {0x2C, 0xA5, 0xB4, 0x2D};
    ...
    for (int i = 0; i < 26; i++)
    {
        schedule_entries[schedule_counter++] =
            generate_schedule_entry(map_list[i], &schedule_data);

        if (map_list[i] == 0x22)
        {
            schedule_entries[schedule_counter++] =
                generate_schedule_entry(map_list[i], &schedule_data, 4);
        }
    }
    ...
    for (int byte5 = 0; byte5 < 256; byte5++)
    {
        for (int byte6 = 0; byte6 < 256; byte6++)
        {
            for (int byte7 = 0; byte7 < 256; byte7++)
            {
                ...
                {
                    working_code[i] =
                        (working_code[i - 8] + run_rng(&rng_seed, rng_table)) &
                        0xFF;
                }
                ...
                */
                ...
                __m256i working_code0 =
                    _mm256_load_si256((__m256i *)(((uint8_t *)working_code)));
                __m256i working_code1 = _mm256_load_si256(
                    (__m256i *)(((uint8_t *)working_code) + 32));
                __m256i working_code2 = _mm256_load_si256(
                    (__m256i *)(((uint8_t *)working_code) + 64));
                __m256i working_code3 = _mm256_load_si256(
                    (__m256i *)(((uint8_t *)working_code) + 96));
                __m256i working_code4 = _mm256_load_si256(
                    (__m256i *)(((uint8_t *)working_code) + 128));
                __m256i working_code5 = _mm256_load_si256(
                    (__m256i *)(((uint8_t *)working_code) + 160));
                __m256i working_code6 = _mm256_load_si256(
                    (__m256i *)(((uint8_t *)working_code) + 192));
                __m256i working_code7 = _mm256_load_si256(
                    (__m256i *)(((uint8_t *)working_code) + 224));

                for (schedule_counter = 0; schedule_counter < 27;
                     schedule_counter++)
                {
                    ...
                    // Next, the working code is processed with the same steps
                    // 16 times:
                    for (int i = 0; i < 16; i++)
                    {
                        _mm256_store_si256(
                            (__m256i *)(((uint8_t *)working_code)),
                            working_code0);

                        // Get the highest bit of the nibble selector to use as
                        // a flag
                        unsigned char nibble = (nibble_selector >> 15) & 0x01;
                        // Shift the nibble selector up one bit
                        nibble_selector = nibble_selector << 1;

                        // If the flag is a 1, get the high nibble of the
                        // current byte Otherwise use the low nibble
                        unsigned char current_byte = (uint8_t)working_code[i];
                        ...
                        {
                            uint8_t *rng_start =
                                alg0_values + ((rng_seed)*128 * 2);

                            __m128i cur_val_lo =
                                _mm256_castsi256_si128(working_code0);
                            cur_val_lo = _mm_slli_epi16(cur_val_lo, 1);
                            __m128i cur_val_hi =
                                _mm256_extractf128_si256(working_code0, 1);
                            cur_val_hi = _mm_slli_epi16(cur_val_hi, 1);
                            working_code0 =
                                _mm256_set_m128i(cur_val_hi, cur_val_lo);
                            __m256i rng_val =
                                _mm256_load_si256((__m256i *)(rng_start));
                            working_code0 = _mm256_castpd_si256(
                                _mm256_or_pd(_mm256_castsi256_pd(working_code0),
                                             _mm256_castsi256_pd(rng_val)));
                            working_code0 = _mm256_castpd_si256(_mm256_and_pd(
                                _mm256_castsi256_pd(working_code0),
                                _mm256_castsi256_pd(mask_FF)));

                            cur_val_lo = _mm256_castsi256_si128(working_code1);
                            cur_val_lo = _mm_slli_epi16(cur_val_lo, 1);
                            cur_val_hi =
                                _mm256_extractf128_si256(working_code1, 1);
                            cur_val_hi = _mm_slli_epi16(cur_val_hi, 1);
                            working_code1 =
                                _mm256_set_m128i(cur_val_hi, cur_val_lo);
                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 32));
                            working_code1 = _mm256_castpd_si256(
                                _mm256_or_pd(_mm256_castsi256_pd(working_code1),
                                             _mm256_castsi256_pd(rng_val)));
                            working_code1 = _mm256_castpd_si256(_mm256_and_pd(
                                _mm256_castsi256_pd(working_code1),
                                _mm256_castsi256_pd(mask_FF)));

                            cur_val_lo = _mm256_castsi256_si128(working_code2);
                            cur_val_lo = _mm_slli_epi16(cur_val_lo, 1);
                            cur_val_hi =
                                _mm256_extractf128_si256(working_code2, 1);
                            cur_val_hi = _mm_slli_epi16(cur_val_hi, 1);
                            working_code2 =
                                _mm256_set_m128i(cur_val_hi, cur_val_lo);
                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 64));
                            working_code2 = _mm256_castpd_si256(
                                _mm256_or_pd(_mm256_castsi256_pd(working_code2),
                                             _mm256_castsi256_pd(rng_val)));
                            working_code2 = _mm256_castpd_si256(_mm256_and_pd(
                                _mm256_castsi256_pd(working_code2),
                                _mm256_castsi256_pd(mask_FF)));

                            cur_val_lo = _mm256_castsi256_si128(working_code3);
                            cur_val_lo = _mm_slli_epi16(cur_val_lo, 1);
                            cur_val_hi =
                                _mm256_extractf128_si256(working_code3, 1);
                            cur_val_hi = _mm_slli_epi16(cur_val_hi, 1);
                            working_code3 =
                                _mm256_set_m128i(cur_val_hi, cur_val_lo);
                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 96));
                            working_code3 = _mm256_castpd_si256(
                                _mm256_or_pd(_mm256_castsi256_pd(working_code3),
                                             _mm256_castsi256_pd(rng_val)));
                            working_code3 = _mm256_castpd_si256(_mm256_and_pd(
                                _mm256_castsi256_pd(working_code3),
                                _mm256_castsi256_pd(mask_FF)));

                            cur_val_lo = _mm256_castsi256_si128(working_code4);
                            cur_val_lo = _mm_slli_epi16(cur_val_lo, 1);
                            cur_val_hi =
                                _mm256_extractf128_si256(working_code4, 1);
                            cur_val_hi = _mm_slli_epi16(cur_val_hi, 1);
                            working_code4 =
                                _mm256_set_m128i(cur_val_hi, cur_val_lo);
                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 128));
                            working_code4 = _mm256_castpd_si256(
                                _mm256_or_pd(_mm256_castsi256_pd(working_code4),
                                             _mm256_castsi256_pd(rng_val)));
                            working_code4 = _mm256_castpd_si256(_mm256_and_pd(
                                _mm256_castsi256_pd(working_code4),
                                _mm256_castsi256_pd(mask_FF)));

                            cur_val_lo = _mm256_castsi256_si128(working_code5);
                            cur_val_lo = _mm_slli_epi16(cur_val_lo, 1);
                            cur_val_hi =
                                _mm256_extractf128_si256(working_code5, 1);
                            cur_val_hi = _mm_slli_epi16(cur_val_hi, 1);
                            working_code5 =
                                _mm256_set_m128i(cur_val_hi, cur_val_lo);
                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 160));
                            working_code5 = _mm256_castpd_si256(
                                _mm256_or_pd(_mm256_castsi256_pd(working_code5),
                                             _mm256_castsi256_pd(rng_val)));
                            working_code5 = _mm256_castpd_si256(_mm256_and_pd(
                                _mm256_castsi256_pd(working_code5),
                                _mm256_castsi256_pd(mask_FF)));

                            cur_val_lo = _mm256_castsi256_si128(working_code6);
                            cur_val_lo = _mm_slli_epi16(cur_val_lo, 1);
                            cur_val_hi =
                                _mm256_extractf128_si256(working_code6, 1);
                            cur_val_hi = _mm_slli_epi16(cur_val_hi, 1);
                            working_code6 =
                                _mm256_set_m128i(cur_val_hi, cur_val_lo);
                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 192));
                            working_code6 = _mm256_castpd_si256(
                                _mm256_or_pd(_mm256_castsi256_pd(working_code6),
                                             _mm256_castsi256_pd(rng_val)));
                            working_code6 = _mm256_castpd_si256(_mm256_and_pd(
                                _mm256_castsi256_pd(working_code6),
                                _mm256_castsi256_pd(mask_FF)));

                            cur_val_lo = _mm256_castsi256_si128(working_code7);
                            cur_val_lo = _mm_slli_epi16(cur_val_lo, 1);
                            cur_val_hi =
                                _mm256_extractf128_si256(working_code7, 1);
                            cur_val_hi = _mm_slli_epi16(cur_val_hi, 1);
                            working_code7 =
                                _mm256_set_m128i(cur_val_hi, cur_val_lo);
                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 224));
                            working_code7 = _mm256_castpd_si256(
                                _mm256_or_pd(_mm256_castsi256_pd(working_code7),
                                             _mm256_castsi256_pd(rng_val)));
                            working_code7 = _mm256_castpd_si256(_mm256_and_pd(
                                _mm256_castsi256_pd(working_code7),
                                _mm256_castsi256_pd(mask_FF)));
                        ...
                        {
                            uint8_t *rng_start =
                                regular_rng_values + ((rng_seed)*128 * 2);
                            ...
                            sum_lo = _mm_add_epi16(
                                _mm256_castsi256_si128(working_code1),
                                _mm256_castsi256_si128(rng_val));
                            ...
                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 64));
                            sum_lo = _mm_add_epi16(
                                _mm256_castsi256_si128(working_code2),
                                _mm256_castsi256_si128(rng_val));
                            sum_hi = _mm_add_epi16(
                                _mm256_extractf128_si256(working_code2, 1),
                                _mm256_extractf128_si256(rng_val, 1));
                            ...
                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 96));
                            sum_lo = _mm_add_epi16(
                                _mm256_castsi256_si128(working_code3),
                                _mm256_castsi256_si128(rng_val));
                            sum_hi = _mm_add_epi16(
                                _mm256_extractf128_si256(working_code3, 1),
                                _mm256_extractf128_si256(rng_val, 1));
                            ...
                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 128));
                            sum_lo = _mm_add_epi16(
                                _mm256_castsi256_si128(working_code4),
                                _mm256_castsi256_si128(rng_val));
                            sum_hi = _mm_add_epi16(
                                _mm256_extractf128_si256(working_code4, 1),
                                _mm256_extractf128_si256(rng_val, 1));
                            ...
                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 160));
                            sum_lo = _mm_add_epi16(
                                _mm256_castsi256_si128(working_code5),
                                _mm256_castsi256_si128(rng_val));
                            sum_hi = _mm_add_epi16(
                                _mm256_extractf128_si256(working_code5, 1),
                                _mm256_extractf128_si256(rng_val, 1));
                            ...
                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 192));
                            sum_lo = _mm_add_epi16(
                                _mm256_castsi256_si128(working_code6),
                                _mm256_castsi256_si128(rng_val));
                            sum_hi = _mm_add_epi16(
                                _mm256_extractf128_si256(working_code6, 1),
                                _mm256_extractf128_si256(rng_val, 1));
                            ...
                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 224));
                            sum_lo = _mm_add_epi16(
                                _mm256_castsi256_si128(working_code7),
                                _mm256_castsi256_si128(rng_val));
                            sum_hi = _mm_add_epi16(
                                _mm256_extractf128_si256(working_code7, 1),
                                _mm256_extractf128_si256(rng_val, 1));
                        ...
                        else if (alg_id == 2)
                        {
                            ...
                            working_code7 = _mm256_castpd_si256(
                                _mm256_or_pd(_mm256_castsi256_pd(part1),
                            ...
                            cur_val_hi =
                                _mm256_extractf128_si256(working_code6, 1);
                            next_carry = _mm256_castpd_si256(_mm256_and_pd(
                                _mm256_castsi256_pd(_mm256_set_m128i(
                                    _mm_slli_si128(cur_val_lo, 14),
                                    cur_val_lo)),
                                _mm256_castsi256_pd(mask_top_01)));
                            ...
                            temp_hi = _mm_srli_epi16(cur_val_hi, 1);
                            temp256 = _mm256_set_m128i(temp_hi, temp_lo);
                            part1 = _mm256_castpd_si256(
                                _mm256_and_pd(_mm256_castsi256_pd(temp256),
                                              _mm256_castsi256_pd(mask_007F)));
                            temp_lo = _mm_slli_epi16(cur_val_lo, 1);
                            temp_hi = _mm_slli_epi16(cur_val_hi, 1);
                            temp256 = _mm256_set_m128i(temp_hi, temp_lo);
                            part2 = _mm256_castpd_si256(
                                _mm256_and_pd(_mm256_castsi256_pd(temp256),
                                              _mm256_castsi256_pd(mask_FE00)));
                        ...
                        else if (alg_id == 3)
                        {
                            ...
                            __m256i rng_val =
                                _mm256_load_si256((__m256i *)(rng_start));
                            working_code0 = _mm256_castpd_si256(_mm256_xor_pd(
                                _mm256_castsi256_pd(working_code0),
                                _mm256_castsi256_pd(rng_val)));

                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 32));
                            working_code1 = _mm256_castpd_si256(_mm256_xor_pd(
                                _mm256_castsi256_pd(working_code1),
                                _mm256_castsi256_pd(rng_val)));

                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 64));
                            working_code2 = _mm256_castpd_si256(_mm256_xor_pd(
                                _mm256_castsi256_pd(working_code2),
                                _mm256_castsi256_pd(rng_val)));

                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 96));
                            working_code3 = _mm256_castpd_si256(_mm256_xor_pd(
                                _mm256_castsi256_pd(working_code3),
                                _mm256_castsi256_pd(rng_val)));

                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 128));
                            working_code4 = _mm256_castpd_si256(_mm256_xor_pd(
                                _mm256_castsi256_pd(working_code4),
                                _mm256_castsi256_pd(rng_val)));

                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 160));
                            working_code5 = _mm256_castpd_si256(_mm256_xor_pd(
                                _mm256_castsi256_pd(working_code5),
                                _mm256_castsi256_pd(rng_val)));

                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 192));
                            working_code6 = _mm256_castpd_si256(_mm256_xor_pd(
                                _mm256_castsi256_pd(working_code6),
                                _mm256_castsi256_pd(rng_val)));

                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 224));
                            working_code7 = _mm256_castpd_si256(_mm256_xor_pd(
                                _mm256_castsi256_pd(working_code7),
                                _mm256_castsi256_pd(rng_val)));
                        ...
                        else if (alg_id == 4)
                        {
                            ...
                            __m256i rng_val =
                                _mm256_load_si256((__m256i *)(rng_start));
                            __m128i sum_lo = _mm_sub_epi16(
                                _mm256_castsi256_si128(working_code0),
                                _mm256_castsi256_si128(rng_val));
                            __m128i sum_hi = _mm_sub_epi16(
                                _mm256_extractf128_si256(working_code0, 1),
                                _mm256_extractf128_si256(rng_val, 1));
                            working_code0 = _mm256_set_m128i(sum_hi, sum_lo);
                            working_code0 = _mm256_castpd_si256(_mm256_and_pd(
                            ...
                            rng_val =
                                _mm256_load_si256((__m256i *)(rng_start + 32));
                            sum_lo = _mm_sub_epi16(
                                _mm256_castsi256_si128(working_code1),
                                _mm256_castsi256_si128(rng_val));
                            sum_hi = _mm_sub_epi16(
                                _mm256_extractf128_si256(working_code1, 1),
                                _mm256_extractf128_si256(rng_val, 1));
                            working_code1 = _mm256_set_m128i(sum_hi, sum_lo);
                            working_code1 = _mm256_castpd_si256(_mm256_and_pd(
                ...
                _mm256_store_si256((__m256i *)(((uint8_t *)working_code)),
                                   working_code0);
                _mm256_store_si256((__m256i *)(((uint8_t *)working_code) + 32),
                                   working_code1);
                _mm256_store_si256((__m256i *)(((uint8_t *)working_code) + 64),
                                   working_code2);
                _mm256_store_si256((__m256i *)(((uint8_t *)working_code) + 96),
                                   working_code3);
                _mm256_store_si256((__m256i *)(((uint8_t *)working_code) + 128),
                                   working_code4);
                _mm256_store_si256((__m256i *)(((uint8_t *)working_code) + 160),
                                   working_code5);
                _mm256_store_si256((__m256i *)(((uint8_t *)working_code) + 192),
                                   working_code6);
                _mm256_store_si256((__m256i *)(((uint8_t *)working_code) + 224),
                                   working_code7);
